    ...
    module Drops
      class Contents < ::Liquid::Drop
        ...
        def before_method(meth)
          type = site.lookup_content_type(meth.to_s)
      ...
      class ProxyCollection < ::Liquid::Drop
        ...
        def before_method(meth)
          ...
          elsif (meth.to_s =~ /^select_values_for_(.+)$/) == 0
        ...
        def collection
          ...
          if @context['with_scope'].blank?
            @collection = @content_type.contents
          else
            @collection = []

            conditions = @context['with_scope'].clone.delete_if { |k, v| %w(per_page page).include?(k) }
